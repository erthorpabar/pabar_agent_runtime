...
async def receive_messages(
    ...
    audio_ctx: dict | None = None  # input_audio_start 时填充
    ...
    while True:
        ...
        if "bytes" in msg and msg["bytes"] is not None:
            # 如果没有 input_audio_start 表明音频没有开始 是异常音频
        ...
        if "text" in msg and msg["text"] is not None:
            ...
            # 1 纯文本输入
            if in_type == "text":
                await in_queue.put(data) # 放入队列
                continue

            # 2 音频开始
            if in_type == "input_audio_start":
                audio_buf.clear()
                meta = (data.get("data") or {})
                audio_ctx = {
                    "channel": data.get("channel", "cmd"),
                    "user_id": data["user_id"],
                    "session_id": data["session_id"],
                    "out_type": data.get("out_type", "audio"),
            ...
            if in_type == "input_audio_end":
                ...
                data = {
                    "channel": audio_ctx["channel"],
                    "user_id": audio_ctx["user_id"],
                    "session_id": audio_ctx["session_id"],
                    "content": text,
                    "in_type": "text",
                    "out_type": audio_ctx["out_type"],
                }
                await in_queue.put(data)
...
async def broadcast_messages(websocket: WebSocket, out_queue: asyncio.Queue):
    ...
    async with aiohttp.ClientSession() as http:
        while True:
            ...
            out_type = result.get("out_type") or "text"
            # 1 文本
            if out_type == "text":
                message = json.dumps(result, ensure_ascii=False)
                try:
            ...
            # 2 音频
                ...
                # 3 结束
                end_msg = {"out_type": "output_audio_end"}
...
async def ws_client_connect(websocket: WebSocket):
    ...
    try:
        async with aiohttp.ClientSession() as http:
            # 3 后台线程
            receive_task = asyncio.create_task(receive_messages(websocket, in_queue, http))
            process_task = asyncio.create_task(agent_loop(in_queue, out_queue))
            broadcast_task = asyncio.create_task(broadcast_messages(websocket, out_queue))
...
2 in_queue -> agent_loop

3 agent_loop -> out_queue
    {
        "type": "status",      # 类型
        "symbol": "thinking",  # 种类
        "content": "hello",    # 具体内容
    ...
    1 开始 text
        {
            ...
            "out_type": "audio",         # 可写 text or audio
...
4 out_queue -> frontend
    ...
    return {
        ...
        "symbol"     # 不同状态
        "content"    # 具体内容
        "out_type": "text",         # 可写 text or audio
    }
...
while True:
    ...
    if "bytes" in msg and msg["bytes"] is not None:
        # 一帧二进制 = 一段 PCM（前端用 send(arrayBuffer) 发来的）
        audio_buf.extend(msg["bytes"])
        continue
    ...
    if "text" in msg and msg["text"] is not None:
        ...
        if mt == "audio_start":
            audio_buf.clear()
            # 可读 data["sample_rate"] 等存到当前连接上下文
            continue

        if mt == "audio_end":
            pcm = bytes(audio_buf)
            audio_buf.clear()
            # 把「一整段 PCM」交给 ASR -> str，再 put 给 agent_loop 能消费的格式
            await in_queue.put({
                "msg_type": "user_audio",  # 整段 pcm 待 ASR；ASR 后再 put msg_type=cmd + content
                "pcm": pcm,  # 或 base64，但 bytes 在队列里没问题
                **{k: data[k] for k in ("user_id", "session_id") if k in data},
            })
            continue
