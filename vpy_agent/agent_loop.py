    ...
    for block in ai_answer:
        if block.type == "tool_use": 
            ...
            if handler is None:
                ...
                try:
                    # 打印输入
                    await out_queue.put({"type": "tool", "symbol": "tool","content": block.input}) # 向前端发送tool_use
                    print(f'[Tool]')
            ...
            # 打印输出
            await out_queue.put({"type": "tool", "symbol": "result","content": output}) # 向前端发送tool_result
            print(f'[Result]')
...
async def agent_loop(in_queue: asyncio.Queue, out_queue: asyncio.Queue):
    ...
    thread_id = f"{data['user_id']}_{data['session_id']}"
...
'''
    ...
    while True:
        ...
        out_type = data.get("out_type", "text")
        ...
        while True:
            ...
            try:
                await out_queue.put({"type": "status", "symbol": "thinking"}) # 向前端发送thinking状态
                ...
                await out_queue.put({"type": "status", "symbol": "api_or_network_error"}) # 向前端发送api_or_network_error状态
            ...
            if res.stop_reason == "end_turn":
                ...
                await out_queue.put({"type": "assistant", "content": ai_answer, "out_type": out_type}) # 向前端发送ai_answer
            ...
            # yes tool_call
                ...
                for block in res.content:
                    if block.type == "text": 
                        ...
                        print(f'({ai_purpose})')
                        await out_queue.put({"type": "tool", "symbol": "purpose","content": ai_purpose}) # 向前端发送tool_call
            ...
            # other
                ...
                print(f"[Max Tokens]")
                await out_queue.put({"type": "status", "symbol": "out of tokens"}) # 向前端发送out of tokens状态
                break
            else:
...
前端协议（与 chat.html 一致）
status -> 渲染连接/思考状态
    symbol: thinking | api_or_network_error | out of tokens
tool -> 首行预览，点击展开全文
    symbol: purpose -> content 模型说明或兜底文案
    symbol: tool   -> content 工具入参（多为 dict，前端 JSON 展示）
    symbol: result -> content 工具执行结果字符串
assistant -> 渲染 AI 最终回复
    content: 纯文本
'''